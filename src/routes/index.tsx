    ...
    <MobileFrame pad={false}>
      ...
      <div className="sticky top-0 z-30 bg-gradient-to-b from-[#FFC97A] via-[#FFB347] to-[#FFA947] pt-3 pb-2 px-3 shadow-sm">
        <div className="flex items-center gap-1 text-[12.5px] text-foreground">
          ...
          <span>
            Deliver to <span className="font-bold">110002</span>
          </span>
        ...
        <div className="mt-2 flex items-center gap-2">
          ...
          <div className="relative">
            ...
            <span className="absolute -top-1 -right-1 h-3.5 w-3.5 rounded-full bg-[#FF3F6C] text-white text-[8px] flex items-center justify-center font-bold">
              3
            </span>
          </div>
        ...
        <div className="mt-3 flex items-center justify-between text-[12.5px] font-semibold">
          ...
          <span className="h-7 w-7 rounded-md bg-[#282C3F] flex items-center justify-center">
        ...
        <div className="mt-3 grid grid-cols-5 gap-2">
          {categories.map((c) => (
            <Link key={c.label} to="/listing" className="flex flex-col items-center gap-1">
              ...
              <span className={`text-[10.5px] ${c.active ? "text-[#FF3F6C] font-bold" : "text-foreground"}`}>
                {c.label}
              </span>
            </Link>
          ))}
      ...
      <div className="bg-white px-4 py-3 flex items-center gap-2 text-[12.5px]">
        ...
        {["06 h", "21 m", "41 s"].map((t) => (
          <span key={t} className="rounded-md bg-[#FFE7DA] text-[#FF905A] font-bold px-2 py-0.5 text-[11px]">
            {t}
          </span>
        ))}
      </div>
      ...
      <div className="px-3">
        <Link to="/listing" className="block relative rounded-2xl overflow-hidden">
          ...
          <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/60 to-transparent">
            <div className="bg-white inline-block px-3 py-1 rounded font-extrabold tracking-wide text-[13px]">
              HIGHLANDER
            </div>
