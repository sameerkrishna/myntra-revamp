...
export const Route = createFileRoute("/cart")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Review your bag with Purchase Confidence powered by Fit Score and friend feedback.",
      },
    ],
  }),
  component: Cart,
  ...
  return (
    <MobileFrame pad={false}>
      ...
      <div className="bg-white sticky top-0 z-30">
        <div className="flex items-center gap-3 px-3 h-12">
          ...
          <div className="flex items-center gap-1 text-[14px]">
            <span className="font-bold">110002</span>
      ...
      <section
        ...
        className="px-3 py-3 space-y-3 scroll-mt-24"
      >
        <div className="flex items-center gap-2">
          <h2 className="text-[15px] font-extrabold">Your Bag</h2>
        </div>
        <div className="flex items-center gap-2 text-[13px]">
          <span className="h-5 w-5 rounded bg-[#FF3F6C] text-white flex items-center justify-center text-[11px]">
            ✓
          </span>
          <span className="font-bold">
            {qty}/{qty} {qty === 1 ? "Item" : "Items"} Selected
          </span>
        ...
        <div className="bg-white rounded-2xl p-3 flex gap-3 border border-border">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-0.5 text-[11.5px] flex items-center gap-1">
              <MLogo className="h-3 w-3" />
        ...
        <div className="bg-white rounded-2xl p-3 border border-border">
          <div className="flex items-center justify-end mb-1">
            <span className="text-[8px] font-bold uppercase tracking-wider bg-[#F9F5E8] text-[#7A5C2F] border border-[#E5D9C3] rounded-full px-1.5 py-[1px]">
              New Feature
            </span>
          </div>
          <div className="flex items-center gap-2 mb-2">
            <span className="h-7 w-7 rounded-full bg-[#F3EEFF] text-[#5A3FBF] flex items-center justify-center">
                ...
                <>
                  ...
                  {suggestNames.length > 0 && (
                    <span className="text-muted-foreground">
                      {" "}
                      {suggestNames.join(", ")} suggested checking fabric thickness.
                    </span>
                  )}
                </>
              ) : (
          ...
          <div className="mt-2 grid grid-cols-2 gap-2">
            <button
              ...
              className="rounded-full border border-[#FF3F6C] text-[#FF3F6C] py-2 text-[12.5px] font-semibold"
            >
              View feedback
            </button>
            <button
              onClick={openAskAgain}
              className="rounded-full bg-[#FFF0F4] text-[#FF3F6C] py-2 text-[12.5px] font-semibold"
            >
              {shop.askedFriends ? "Ask again" : "Ask friends"}
            </button>
          </div>
        </div>
            ...
            {[
              ...
              <div key={c.brand} className="rounded-lg border border-border overflow-hidden">
                <div className="relative bg-secondary aspect-square">
                  ...
                  <button className="absolute bottom-1 right-1 bg-white border border-[#FF3F6C] text-[#FF3F6C] text-[10px] font-bold px-2 py-0.5 rounded">
                    Add
                  </button>
                </div>
                <div className="p-1.5">
      ...
      <section
        ...
        className="px-3 py-3 space-y-3 scroll-mt-24 border-t border-border"
      >
        ...
        <div className="bg-white rounded-2xl border border-border p-3">
          <div className="flex items-center justify-between">
            <div className="text-[13.5px] font-bold leading-snug">
              Donate &amp; Support Transformative Social Work In India
            </div>
            <button className="text-[12px] underline decoration-dotted font-semibold">Know More</button>
          </div>
          <div className="mt-2 flex gap-2 flex-wrap">
            {["₹10", "₹20", "₹50", "₹80", "₹100"].map((v) => (
              <button key={v} className="rounded-full border border-border text-[12.5px] px-3 py-1.5 font-semibold">
                {v}
              </button>
            ))}
          </div>
        ...
        <div className="bg-white rounded-2xl border border-border overflow-hidden">
          ...
          <div className="p-3 flex items-center gap-3 border-t border-border">
            <span className="h-9 w-9 rounded bg-[#F0EAFA] text-[#7A56D6] flex items-center justify-center font-extrabold">
              🏷️
            </span>
            <div className="flex-1">
              ...
              <div className="text-[11px] text-muted-foreground">
                15% off upto Rs. 200 on minimum purchase of Rs. 1099
              </div>
            </div>
            <button
              onClick={() => toast.success("Coupon applied — ₹188 off")}
              className="rounded-md border border-border text-[12.5px] font-bold px-3 py-1.5"
            >
              Apply
            </button>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-border p-3 flex items-center gap-3">
          <span className="h-9 w-9 rounded bg-[#FFE7EE] text-[#F13AB1] flex items-center justify-center font-extrabold text-[10px]">
            GST
          </span>
          <div className="flex-1">
            <div className="text-[13px] font-extrabold flex items-center gap-1.5">
              ADD GSTIN <span className="text-[9px] bg-[#FF3F6C] text-white px-1.5 py-0.5 rounded">NEW</span>
            </div>
      ...
      >
        ...
        <div className="bg-white rounded-2xl border border-border p-4 text-[13.5px] space-y-2">
          ...
          <div className="flex justify-between">
            ...
            <span className="text-[#03A685]">-₹{discount.toLocaleString("en-IN")}</span>
          </div>
          <div className="flex justify-between">
            <span>
              Platform Fee{" "}
              <span className="underline decoration-dotted text-muted-foreground text-[12px]">Know More</span>
            </span>
            <span>₹{platform}</span>
          </div>
        ...
        <div className="rounded-2xl overflow-hidden bg-gradient-to-r from-[#9B5BD6] to-[#3F7CD6] p-4 text-white flex items-center gap-3">
          ...
          <button className="bg-white text-foreground font-bold text-[12.5px] rounded-md px-3 py-2 flex items-center gap-1">
        ...
        <p className="text-[11.5px] text-muted-foreground leading-snug pb-2">
          By placing the order, you agree to Myntra's <span className="text-[#FF3F6C] font-bold">Terms of Use</span> and{" "}
      ...
      <div className="sticky bottom-0 left-0 right-0 z-40 bg-white border-t border-border shadow-[0_-4px_12px_rgba(0,0,0,0.06)]">
        <div className="bg-[#FFF0F4] text-center text-[12px] py-1.5 font-semibold">
          {qty} {qty === 1 ? "Item" : "Items"} selected for order
        </div>
        <button
          onClick={() =>
            toast.success("Prototype complete: Order placed.", {
              description: "Thanks for trying the Fit Score + Ask Friends flow.",
            })
          }
      ...
      <AskFriendsSheet
        key={askInitial + String(askOpen)}
        open={askOpen}
        onOpenChange={setAskOpen}
        initialStep={askInitial}
      />
    </MobileFrame>
  );
}
