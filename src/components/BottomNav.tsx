      ...
      {items.map((i) => {
        ...
        return (
          <Link
            key={i.key}
            to={i.to}
            className="flex flex-col items-center justify-center gap-0.5 pt-1"
          >
            {i.key === "home" && <MLogo className="h-5 w-5" />}
            {i.key === "fwd" && (
              <span className="text-[15px] font-extrabold italic" style={{ color: active ? "#FF3F6C" : "#282C3F" }}>fwd</span>
            )}
            {i.key === "mnow" && <MLogo className="h-5 w-5" />}
            {i.key === "luxe" && <span className="text-[11px] tracking-[0.2em] font-semibold">LUXE</span>}
            {i.key === "bag" && <ShoppingBag className="h-5 w-5" strokeWidth={2} color={active ? "#FF3F6C" : "#282C3F"} />}
            <span className={active ? "text-[#FF3F6C] font-semibold" : "text-foreground"}>{i.key === "home" || i.key === "bag" ? i.label : i.sub}</span>
